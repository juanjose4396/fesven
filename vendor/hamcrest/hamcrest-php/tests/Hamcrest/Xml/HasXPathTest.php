...
HTML;
    ...
    {
        return \Hamcrest\Xml\HasXPath::hasXPath('/admin/user');
    }
    ...
    public function testHasAReadableDescription()
    {
        $this->assertDescription(
            'XML or HTML document with XPath "/admin/user"',
            hasXPath('/admin/user')
        );
        $this->assertDescription(
            'XML or HTML document with XPath "count(/admin/user)" <2>',
            hasXPath('/admin/user', 2)
        );
        $this->assertDescription(
            'XML or HTML document with XPath "/admin/user/name"'
            . ' a string starting with "Alice"',
            hasXPath('/admin/user/name', startsWith('Alice'))
        );
    }
